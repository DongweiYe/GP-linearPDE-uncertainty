...
max_samples = 1000
assumption_sigma = 0.03  # step size
k = 0.6
num_chains = 1
...
if __name__ == '__main__':
    ...
    def generate_prior_samples(rng_key, num_samples, prior_mean, prior_cov):
        ...
        prior_samples = 2 * (prior_samples - min_val) / (max_val - min_val) - 1
    ...
    prior_rng_key, prior_key = random.split(random.PRNGKey(59))
    ...
    plot_u_pred_rd(Xu_all_without_noise, Xu_certain, Xf, Xu_noise, noise_std, Xu_pred_mean, prior_var, assumption_sigma, k,
                max_samples, learning, num_chains, number_f, added_text)
    plot_dist_rd(Xu_all_without_noise, Xu_certain, Xf, Xu_noise, noise_std, Xu_pred_mean, prior_var, assumption_sigma, k,
              max_samples, learning, num_chains, number_f, posterior_samples_list, prior_samples, number_u, added_text)
    #plot_with_noise(number_u, 0, posterior_samples_list, prior_samples, Xu_certain, Xu_noise, bw, added_text)
