...
def figure1_posterior(prior_mean,prior_var,posterior_samples,groundtruth):
    ...
    for i in range(prior_mean.shape[0]):
        ...
        ### Plot prior
        plt.fill_between(prior_range, norm.pdf(prior_range, prior_mean[i], prior_var[i]),color='tab:orange',alpha=0.3,linewidth=0.1,label='prior')
        # sns.kdeplot(norm.pdf(prior_range, prior_mean[i], prior_var[i]),color='tab:blue',bw_adjust=3,linewidth=0,fill=True,alpha=0.3,label='prior')
        # plt.axvline(x=prior_mean[i],color='tab:green',linestyle='--',linewidth=3)

        ### Plot posterior
        s1 = sns.kdeplot(posterior_samples[:,i],color='tab:blue',bw_adjust=3,linewidth=0,fill=True,alpha=0.3,label='posterior')
        plt.axvline(x=np.mean(posterior_samples[:,i]),color='tab:blue',linestyle='--',linewidth=6,label='posterior mean')
        s1.set(ylabel='')
        plt.xlabel('x')
        if i == 0:
